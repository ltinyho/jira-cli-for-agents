...
type Config struct {
	...
	Email             string            `yaml:"email"`                         // User email for API token (Cloud) or username (Data Center)
	APIToken          string            `yaml:"api_token,omitempty"`           // Jira API token (deprecated: use keyring)
	AuthType          string            `yaml:"auth_type,omitempty"`           // "basic" (default, email+token) or "bearer" (PAT for Data Center)
...
func (c *Config) GetBaseURL() string {
	return fmt.Sprintf("https://%s/rest/api/2", c.Domain)
}
