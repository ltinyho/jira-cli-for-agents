...
cleanup() {
    if [[ -n "$TEMP_DIR" && -d "$TEMP_DIR" && "$TEMP_DIR" != *"jira-cli-skill-cache"* ]]; then
        rm -rf "$TEMP_DIR"
    fi
}
trap cleanup EXIT
...
locate_skill_source() {
    ...
    # 3. 从 GitHub clone（用持久化 cache 目录以支持 symlink 模式）
    log_info "本地未找到 skill 源，从 GitHub clone..."
    CACHE_DIR="$HOME/.cache/jira-cli-skill-cache"
    TEMP_DIR="$CACHE_DIR"
    mkdir -p "$CACHE_DIR"
    if command -v git >/dev/null 2>&1; then
        if [[ ! -d "$CACHE_DIR/repo" ]]; then
            git clone --depth 1 "$REPO_URL" "$CACHE_DIR/repo" 2>&1 | sed 's/^/  /' >&2 || {
                log_error "git clone 失败，请检查网络或 SSH key"
                exit 1
            }
        else
            log_info "  使用缓存: $CACHE_DIR/repo（执行 git pull 更新）"
            (cd "$CACHE_DIR/repo" && git pull --ff-only 2>&1 | sed 's/^/  /') >&2 || true
        fi
        echo "$CACHE_DIR/repo/.opencode/skills/$SKILL_NAME"
        return
    fi
...
if [[ $USE_LINK -eq 1 ]]; then
    # 转为绝对路径，避免 symlink 在不同 cwd 解析失败
    SOURCE_ABS=$(cd "$SOURCE" 2>/dev/null && pwd || echo "$SOURCE")
    ln -s "$SOURCE_ABS" "$TARGET"
    log_ok "已创建 symlink: $TARGET -> $SOURCE_ABS"
